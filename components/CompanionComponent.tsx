...
const CompanionComponent = ({ companionId, name, topic, title, duration, subject, username, userImage, style, voice }: CompanionComponentProps) => {
    ...
    return (
        <section className="flex flex-col h-[68vh]">
            <section className="flex gap-8 max-sm:flex-col">
                ...
                <div className="user-section">
                    <div className="user-avatar">
                        <Image src={userImage} alt={userImage} width={90} height={90} className="rounded-lg" />
                    <p className="font-bold text-2xl">
                        {username}
                    </p>
                    </div>
                    ...
                    <button className={cn("rounded-lg py-2 cursor-pointer transition-colors w-full text-white",
                        ...
                        callStatus === CallStatus.CONNECTING && 'animate-pulse')} onClick={callStatus === CallStatus.ACTIVE ?
            ...
            <section className="transcript">
                <div className="transcript-message no-scrollbar">
                    {messages.length > 0 && messages.map((message, index) => {
                        ...
                        return (
                            <p key={index} className={cn("max-sm:text-sm", isAssistant ? "" : "text-primary")}>
                                {isAssistant
